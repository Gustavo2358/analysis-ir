...
Na fronteira de `k`, os sujeitos da assinatura DEVEM identificar a entrada ou a assinatura externa materializada em `k`, direção, posição e papel de chamado; objetos/operandos do chamador conservam seu próprio papel. A notação semântica `parameter(k,e,i)` / `result(k,e,i)` distingue a instância de assinatura de `parameter(e,i)` / `result(e,i)` usada na declaração da entrada e no corpo. Para contrato externo, usam-se `external_parameter(k,i)` / `external_result(k,i)`: a assinatura pertence a `k`, não à grafia de autoridade/versão de `ContractRef`. Dois sites com a mesma evidência contratual continuam sujeitos distintos. A posição referida DEVE estar materializada, inclusive quando seu `TypeRef` é desconhecido; um restante de assinatura não cria posições referenciáveis por adivinhação. Se o alvo admitir várias assinaturas, a garantia normalizada e a prova DEVEM cobrir cada vínculo possível; não selecionam um alvo nem fecham um restante desconhecido.
