...
Uma chamada externa sem contrato suficiente DEVE admitir os resultados não excluídos por sua forma semântica: normalmente retorno, saída excepcional, término e divergência. Quando saltos não locais ou reentradas não podem ser excluídos, deve incluir `open(scope)` apropriado. O núcleo não afirma que toda chamada desconhecida só retorna normalmente.
