...
Na migração, um domínio `T` estabelecido passa a `known(T)`, inclusive tipos de extensão. Um fato realmente sem domínio estabelecido passa a `unknown_type(u)` com lacuna explícita; não se renomeia um domínio de extensão para representar essa ausência. Construções antes aceitas por dispensar precondições sob desconhecimento precisam de abstração com envelopes e operandos preservados, ou de fatos de tipo já sustentados. Publicações devem ser revalidadas; um consumidor 1.0.0 não recebe garantia automática de fallback para a nova variante.
