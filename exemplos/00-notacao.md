...
Cada cenário X-01 a X-30 é uma publicação independente. Variantes dentro do mesmo cenário são explicitamente independentes quando indicado. As consultas escritas após os blocos são resultados de consumers, **não conteúdo da IR**.
...
| `unknown(T)` | Valor desconhecido de `T`, sem escritas/controle oculto |
...
Todo uso abreviado de `unknown(T)` tem razão `SOURCE_SEMANTICS_UNAVAILABLE`, lista de leituras vazia e `remainingReads=none`, salvo dependências explicitamente fornecidas. Isso é valor não determinado, não ausência de expressão.

## 3. Declarações de células

`cell @x : T` abrevia uma declaração nominal de tipo T associada a uma célula própria com duração `activation`. **Nos exemplos que a usam**, a independência em relação às demais células é uma premissa expressa do cenário. Ela não pode ser generalizada para declarações reais apenas por nomes diferentes.
...
Para outras categorias, o namespace é `example.<categoria>` e a ação deve ser indicada. A notação `results=[@x]` usa assinatura externa compatível com o tipo de x; seus resultados só são escritos no retorno normal.
